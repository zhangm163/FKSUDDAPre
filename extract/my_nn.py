...
def run_a_contrastive_pretrain_epoch(args, epoch, model, data_loader,
    ...
    for batch_id, batch_data in enumerate(data_loader):
        ...
        optimizer.zero_grad()
        print(f"Epoch {epoch}: Running training...")
        try:
            loss.backward()
            optimizer.step()
...
class EarlyStopping(object):
    def __init__(self, pretrained_model='Null_early_stop.pth',
        ...
        self.counter = 0
        self.filename = 'extract/model/{}_early_stop.pth'.format(task_name)
        self.pretrain_save_filename = 'extract/model/pretrain_{}_epoch_'.format(task_name)
        self.best_score = None
        self.early_stop = False
        self.pretrained_model = 'extract/model/{}'.format(pretrained_model)
