...
def main(data_path, results_output_path, models_to_use, epochs=20, batch_size=64, learning_rate=0.001, n_splits=10, early_stopping_patience=5):
    ...
    for train_index, test_index in kf.split(X):
        ...
        y_true = all_y_true[0]
        y_pred_ensemble = np.array(all_y_pred).mean(axis=0) >= 0.5
        y_pred_prob_ensemble = np.array(all_y_pred_prob).mean(axis=0)
        acc, auc_score, aupr, f1, mcc, sn, sp, precision_val, recall_val = evaluate_model(y_true, y_pred_prob_ensemble, y_pred_ensemble)
        results.append({
            'Model': model_name,
            'Fold': fold,
            'Accuracy (ACC)': acc,
