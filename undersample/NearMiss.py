...
def nearmiss_resample(data, labels, sampling_strategy, version=1):
    nm = NearMiss(version=version, sampling_strategy=sampling_strategy)
    resampled_data, resampled_labels = nm.fit_resample(data, labels)
    return resampled_data, resampled_labels

def group_downsampling_with_nearmiss(df, group_sizes, version=1):
    resampled_data = []
    resampled_labels = []
    for group, size in zip(df['drug_group'].unique(), group_sizes):
        ...
        X_resampled, y_resampled = nearmiss_resample(group_data_features, group_data_labels, sampling_strategy, version)
        ...
        group_data_resampled['disease'] = y_resampled
...
def save_resampled_data(resampled_data):
    try:
        ...
        resampled_data.to_csv('./data/undersample/disAssociation/disNearMiss.csv', index=False)
...
if __name__ == '__main__':
    ...
    try:
        resampled_data, resampled_labels = group_downsampling_with_nearmiss(final_reordered, group_sizes, version=1)
