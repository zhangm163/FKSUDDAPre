...
def group_downsampling_with_ClusterCentroids(df, group_sizes, feature_columns, n_init=10, voting='auto'):
    resampled_data = []
    resampled_labels = []
    for group, size in zip(df['drug_group'].unique(), group_sizes):
        ...
        X_resampled, y_resampled = Cluster_Centroids(group_data_features, group_data_labels, sampling_strategy, n_init,voting)
...
def save_resampled_data(resampled_data):
    try:
        ...
        resampled_data.to_csv('./data/undersample/disAssociation/disClusterCentroids.csv', index=False)
...
if __name__ == '__main__':
    ...
    try:
        resampled_data, resampled_labels = group_downsampling_with_ClusterCentroids(final_reordered, group_sizes,feature_columns, n_init=10,voting='auto')
